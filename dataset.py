...
if __name__ == '__main__':
    ...
    for gl_sc in unique:
        ...
        for idx, val in tqdm(p.iterrows()):
            ...
            image_path = input_path + f_name + '.tiff'
            print("path", image_path)
            image = read_image(image_path, 1)

            mask_path = mask_path + f_name + '_mask.tiff'
            ...
            for iidx, (v, y, x) in enumerate(coords):
                ...
                end_point = (X, Y)
                try:
                    print("write", output_path + str(gl_sc) + '/' + f_name + '_p_' + str(iidx) + '.tiff')
